...
DEFAULT_DOWNLOAD_DIR = Path(r"D:\Papa\YouTube Interviews")
...
def build_ydl_opts(output_dir: Path, cookies_browser: str | None = None) -> dict:
    opts = {
        # True highest quality: best video + best audio (any format), then merge.
...
def list_available_formats(url: str, ydl_opts: dict) -> None:
    ...
    if requested:
        video = requested[0]
        audio = requested[1] if len(requested) > 1 else None
        print("\nSelected for download (highest quality):")
        print(
            f"  Video: {video.get('height')}p | {video.get('ext')} | "
            f"{video.get('vcodec', 'unknown')} | format {video.get('format_id')}"
        )
        if audio:
            print(
                f"  Audio: {audio.get('ext')} | {audio.get('acodec', 'unknown')} | "
                f"~{audio.get('abr', 'N/A')} kbps | format {audio.get('format_id')}"
            )
        print("  Output: merged MP4")
